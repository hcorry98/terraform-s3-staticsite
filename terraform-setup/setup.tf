terraform {
  ...
  backend "s3" {
    ...
    key            = "terraform-aws-s3staticsite/setup.tfstate"
    region         = "us-west-2"
  }
  required_providers {
    aws = {
...
locals {
  name    = "terraform-staticsite"
  gh_org  = "byu-oit"
  gh_repo = "terraform-aws-s3staticsite"
  env     = "dev"
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      repo                   = "https://github.com/byu-oit/terraform-aws-s3staticsite"
